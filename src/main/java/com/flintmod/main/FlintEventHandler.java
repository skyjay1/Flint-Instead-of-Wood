...
{
	private static final Item[] WOOD_TOOLS = { Items.WOODEN_AXE, Items.WOODEN_HOE, Items.WOODEN_PICKAXE, Items.WOODEN_SHOVEL, Items.WOODEN_SWORD };
	private static final Item[] FLINT_TOOLS = { FlintItemInit.FLINT_AXE, FlintItemInit.FLINT_HOE, FlintItemInit.FLINT_PICK, FlintItemInit.FLINT_SHOVEL, FlintItemInit.FLINT_SWORD };
		...
		while(true)
		{
			...
			for(int i = 0, l = WOOD_TOOLS.length; i < l; i++)
			{
				...
				if(oldEntry != null)
				{
					...
					int weight = 3;
					int quality = 1;
					LootEntry newEntry = new LootEntryItem(replacement, weight, quality, new LootFunction[] {}, new LootCondition[] {}, replacement.getRegistryName().toString());
